...
function CerQ (id, uid_, ePoints, out_link) {
	...
	const isTouchDevice = !!('ontouchstart' in window),
		...
		infoText = isTouchDevice ? ["滑動進行 360° 觀看<br>雙指外撥以放大檢視", "Swipe for a 360° view<br>Pinch out to zoom in"] : ["拖曳進行 360° 觀看<br>向上捲動以放大檢視", "Drag for a 360° view<br>Scroll up to zoom in"];

	var cerq = document.getElementById(id); //主要物件節點
	cerq.style.cssText = 'position:relative;overflow:hidden;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;max-width:' + 50 * cerq.firstElementChild.width + 'px;text-align:center;font:18px Helvetica,STHeiti,sans-serif;';
	...
	var m1 = 7*mz/8;
	img1 = new Image();
	img1.src = serialPathN( subpath, m1+1);
	...
	cerq.run = function() { //原始結構請參考 _prototype_cerq.run.js
		...
		function imgLoadRecur( n, dofunction1 ) {
			for (; i[n] != iz[n]; i[n]+=2){
				...
				imgNormal[ i[n] ].src = serialPathN( subpath, i[n] + 1);
	...
	function serialPathN (pathM, n) {
		return "../pic/" + uid + "/" + uid + pathM + "/" + String( "000" + n ).slice(-3) + ".jpg"
		// 切換檔案來源
		//return "https://dl.dropboxusercontent.com/u/28729972/cerqPic/" + uid + "/" + uid + pathM + "/" + String( "000" + n ).slice(-3) + ".jpg"
	}
			...
			if (zoomFactor <= 1) {
				...
				if (zoomFactor > 1.2) {
					...
					if (img1) {
						...
						img1 = imgHigh[_m]
						img1.src = serialPathN("-p1800", _m + 1);
